...
def card_game():
    ...
    try:
        my_sessions = (
            GameSession.query.filter_by(creator_id=current_user.id)
            .order_by(GameSession.created_at.desc())
            .all()
        )
    except Exception as error:
        app.logger.exception("Failed to load card game sessions created by user: %s", error)
        flash('Не удалось загрузить ваши игры. Попробуйте обновить страницу.', 'danger')
        my_sessions = []
