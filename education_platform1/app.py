...
from models import db, User, TestResult, PhysicalEducationResult, Schedule, Homework, LearningMaterial, TrainingProgram, NutritionDiary, Recipe, FitnessGame, Message, GameSession, GameCard, GameParticipant, GameAnswer
...
def card_game():
    ...
    # ⭐ ИСПРАВЛЕНО: Сессии, где я УЖЕ активный участник (НЕ invited)
    active_participations = GameParticipant.query.filter(
        GameParticipant.user_id == current_user.id,
        GameParticipant.status.in_(['joined', 'ready', 'completed'])  # Только активные
    ).all()
    active_session_ids = [p.session_id for p in active_participations]
    joined_sessions = GameSession.query.filter(
        GameSession.id.in_(active_session_ids)
    ).order_by(GameSession.created_at.desc()).all() if active_session_ids else []
    
    # ⭐ ДОБАВЛЕНО: Приглашения (status='invited')
    invited_participations = GameParticipant.query.filter(
        GameParticipant.user_id == current_user.id,
        GameParticipant.status == 'invited'
    ).all()
    invited_session_ids = [p.session_id for p in invited_participations]
    invited_sessions = GameSession.query.filter(
        GameSession.id.in_(invited_session_ids)
    ).order_by(GameSession.created_at.desc()).all() if invited_session_ids else []
    
    return render_template('card_game.html', 
                         my_sessions=my_sessions, 
                         joined_sessions=joined_sessions,  # ⭐ Правильное имя!
                         invited_sessions=invited_sessions)  # ⭐ Приглашения
    ...
    if request.method == 'POST':
        ...
        if not title or not subject or not topic:
            if request.is_json:
                ...
                return redirect(url_for('create_card_game'))
        ...
        # ⭐⭐⭐ КРИТИЧЕСКИ ВАЖНО: Правильный URL!
        if request.is_json:
            return jsonify({
                'success': True, 
                'session_id': session.id,
                'redirect_url': f'/card-game/session/{session.id}'  # БЕЗ /play!
            })
        else:
            return redirect(f'/card-game/session/{session.id}')  # БЕЗ /play!
    ...
    try:
        ...
        with GigaChat(
            ...
            if session.material_text:
                prompt = f"""Ты - эксперт по образованию. Создай {session.num_cards} карточек для обучения по материалу.
...
{{
  "cards": [
    {{
      ...
      "explanation": "Подробное объяснение с деталями из материала"
    }}
  ]
}}
...
- Ответы должны быть конкретными (2-3 предложения)
- Объяснения должны быть подробными (3-5 предложений)

Верни ТОЛЬКО JSON без дополнительного текста:"""
            else:
                prompt = f"""Создай {session.num_cards} карточек для изучения темы.
...
- Объяснения подробные (3-5 предложений)
...
Верни ТОЛЬКО JSON:"""
            ...
            print(f"[INFO] GigaChat ответ получен, длина: {len(content)}")
            ...
            if json_start == -1 or json_end <= json_start:
                print(f"[ERROR] Нет JSON в ответе: {content[:200]}")
                return jsonify({'success': False, 'error': 'GigaChat не вернул JSON формат'}), 500
...
def start_card_game(session_id):
    ...
    # ⭐ ИСПРАВЛЕНО: Получаем ВСЕХ активных участников (включая создателя)
    participants = GameParticipant.query.filter(
        GameParticipant.session_id == session_id,
        GameParticipant.status.in_(['ready', 'joined'])  # Оба статуса
    ).all()
    ...
    if len(participants) < 2:
        return jsonify({'success': False, 'error': f'Нужно минимум 2 участника. Сейчас: {len(participants)}'}), 400
    
    # ⭐ ИСПРАВЛЕНО: Перемешать участников для справедливого распределения
    import random
    random.shuffle(participants)
    
    # ⭐ ИСПРАВЛЕНО: Очистить старые назначения (если запускаем повторно)
    for card in cards:
        card.assigned_to_user_id = None
    db.session.commit()
    
    # Распределить карточки между участниками по кругу
    for i, card in enumerate(cards):
        participant = participants[i % len(participants)]
        card.assigned_to_user_id = participant.user_id
        print(f"[DEBUG] Карточка {card.id} (#{i+1}) → {participant.user.username} (ID: {participant.user_id})")
    ...
    print(f"[OK] Игра началась! Карточек: {len(cards)}, Участников: {len(participants)}")
    return jsonify({
        'success': True, 
        'message': f'Игра началась! Карточек распределено: {len(cards)} между {len(participants)} участниками'
    })
...
def play_card_game(session_id):
    ...
    # ⭐ ИСПРАВЛЕНО: Получить МОИ карточки (назначенные МНЕ)
    my_cards = GameCard.query.filter_by(
        session_id=session_id, 
        assigned_to_user_id=current_user.id
    ).order_by(GameCard.order_index).all()
    
    # ⭐ ИСПРАВЛЕНО: Получить карточки ДРУГИХ участников (НЕ мои)
    cards_to_check = GameCard.query.filter(
        GameCard.session_id == session_id,
        GameCard.assigned_to_user_id != current_user.id,
        GameCard.assigned_to_user_id.isnot(None)  # Только назначенные
    ).order_by(GameCard.order_index).all()
    
    print(f"[DEBUG] Пользователь {current_user.username}:")
    print(f"  - Моих карточек: {len(my_cards)}")
    print(f"  - Карточек для проверки: {len(cards_to_check)}")
    
    for card in my_cards:
        print(f"    МОЯ: #{card.order_index} - {card.question[:50]}...")
    
    for card in cards_to_check:
        assigned_user = card.assigned_to.username if card.assigned_to else "???"
        print(f"    ПРОВЕРЯЮ: #{card.order_index} (отвечает: {assigned_user}) - {card.question[:50]}...")
...
@app.route('/api/card-game/session/<int:session_id>/accept-invite', methods=['POST'])
@login_required
def accept_card_game_invite(session_id):
...
@app.route('/api/card-game/session/<int:session_id>/decline-invite', methods=['POST'])
@login_required
def decline_card_game_invite(session_id):
    ...
    return jsonify({'success': True, 'message': 'Приглашение отклонено'})
