export const situp = {
    ...
    svgIcons: {
        ...
        torsoUp: '<svg width="56" height="56" viewBox="0 0 64 64" fill="white"><path d="M32 45 Q32 25 32 15" stroke="white" stroke-width="4" fill="none"/><circle cx="32" cy="12" r="5"/><path d="M20 35 L32 25 L44 35" stroke="white" stroke-width="3" fill="none"/></svg>',
    ...
    getInitialState() {
        return { position: 'down', hadError: false, errorMsg: '' };
    },
    ...
    analyze(lm, state, showHint, logError, calcAngle) {
        ...
        if (hipAngle < this.thresholds.hipAngleUp && state.position === 'down') {
            ...
            if (hipAngle > this.thresholds.hipAngleGood) {
                ...
                state.hadError = true;
                state.errorMsg = 'Недостаточная высота подъема корпуса';
            } else {
            ...
            showHint('Готов к следующему', this.svgIcons.torsoUp, 'rgba(16, 185, 129, 0.95)', 1500);
            result.status = 'Готов';
